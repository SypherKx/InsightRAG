...
class QueryProcessor:
    ...
    def classify_intent(cls, query: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict with intent: 'visual', 'factual', 'analytical', 'lookup',
            and recommended top_k candidate count.
        """
        q_lower = query.lower()
        ...
        if is_visual:
            return {"intent": "visual", "top_k": 4, "is_visual": True}
        
        if len(words) <= 5 and any(w in words for w in ["what", "who", "when", "where", "define"]):
            return {"intent": "factual", "top_k": 3, "is_visual": False}
            
        if any(w in words for w in ["compare", "difference", "explain", "why", "how", "analyze", "summarize"]):
            return {"intent": "analytical", "top_k": 4, "is_visual": False}
            
        if re.search(r'\b(section|chapter|part|model|v\d+|\d+\.\d+)\b', q_lower):
            return {"intent": "lookup", "top_k": 3, "is_visual": False}

        return {"intent": "standard", "top_k": 4, "is_visual": False}
